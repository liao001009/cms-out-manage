...
import { $reduceCriteria } from '@/desktop/data-base-shared/criteria'
import Operation from '@elem/operation'
import Table, { useTable } from '@elem/mk-table'
import api from '@/api/cmsLevelInfo'
import { useAdd } from '@/desktop/data-base-shared/add'
import { $deleteAll } from '@/desktop/data-base-shared/deleteAll'
import './index.scss'
...
const Content: React.FC<IContentViewProps> = (props) => {
  ...
  //新建
  const { $add: $add } = useAdd('/cmsLevelInfo/add')
  ...
  const handleSearch = useCallback((keyword: string) => {
    queryChange({
      ...
      conditions: {
        ...query.conditions,
        fdLevelName: { $contains: keyword.trim() }
      }
    })
  }, [])
  ...
  const handleCriteriaChange = useCallback(
    (value, values) => {
      ...
      queryChange &&
        queryChange({
          ...query,
          conditions: values[0].value.length ? {
  ...
  const onRowClick = useCallback(
    (record) => {
      return {
        onClick: () => {
          history.goto(`/cmsLevelInfo/view/${record.fdId}`)
        }
      }
    },
    [history]
        ...
        <div className="lui-template-list-criteria">
          ...
          <div className="right">
            ...
            <Criteria key="criteria" onChange={handleCriteriaChange}>
              <Criteria.Input name="fdFrame" title="框架类型"></Criteria.Input>
            </Criteria>
          </div>
        </div>
        <div className="lui-template-list-toolbar">
          <div className="left">
            <Operation key="operation" onChange={handleSorter}>
              {/* 排序 */}
              <Operation.SortGroup>
                <Operation.Sort key="fdOrder" name="fdOrder" title="排序号"></Operation.Sort>
              </Operation.SortGroup>
            </Operation>
          </div>
          <div className="right">
